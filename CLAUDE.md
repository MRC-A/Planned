...
- **Drag to reschedule (F7)** — `editable` plus `@fullcalendar/interaction`'s `eventDrop`/`eventResize`. Only the right (due-date) edge resizes by default (`eventResizableFromStart` stays off, FullCalendar's own default).
  - `dropPatch(task, deltaDays)` — a plain move — shifts whichever of `startDate`/`dueDate` the task actually has by the *same* delta, mirroring `GanttView`'s own drag-to-move, so a task with only a due date doesn't gain a start date it never had and a multi-day span isn't distorted.
  - `resizePatch(start, end)` sets **both** dates explicitly — unlike a move, a resize inherently defines a new range. `end` is FullCalendar's exclusive all-day end, so it's converted back to an inclusive due date the same way `toEvents` builds it, just in reverse.
  - `deltaDays` is computed via this app's own `daysBetween(oldEvent.start, event.start)`, not FullCalendar's `Duration` decomposition (`delta.years`/`.months`/`.days`) — keeps the one place this app does date arithmetic in one place, and sidesteps having to reason about what a `Duration` means across a DST transition.
  - Both handlers call `info.revert()` on a failed `onEdit` (e.g. a network error), snapping the event back visually rather than leaving it in a position the stored data doesn't match.
  - **Testing this live: drag distances must stay inside the visible week.** A synthetic drag that pushes the drop point past the grid's last column (same row, no matching day cell under the cursor) is silently reverted by FullCalendar itself — no `eventDrop` call, no error, just a snap-back — which looks identical to the feature being broken. Confirmed by instrumenting raw `pointerdown`/`pointermove` counts and the `.fc-event-dragging` class during a held drag: the drag machinery was firing correctly the whole time, only the test's own distance was wrong. Anchor test drags near the start of the week with a delta small enough to land in-bounds.
