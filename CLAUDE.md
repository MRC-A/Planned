...
- **Gantt** (`frontend/src/views/GanttView.tsx`) — `frappe-gantt` (imperative vanilla-JS lib, no React binding — mounted into a ref'd `<div>` via `useEffect`, updated in place with `.refresh()` rather than recreated). Chosen over `gantt-task-react` because its CSS uses real custom properties (`--g-*`, see `src/styles/gantt.css`) instead of hashed CSS-module classnames, so it can actually be themed to match the app. No shipped TS types — see the shim in `frontend/src/types/frappe-gantt.d.ts`. Same date/dependency-availability rule as Calendar: undated tasks, or tasks depending on an undated task, are dropped rather than drawn broken.
